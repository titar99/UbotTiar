import os

API_ID = int(os.getenv("API_ID", "24300557")) 
API_HASH = os.getenv("API_HASH", "8a63cf3365dc553dbeab617d03776522") 
BOT_TOKEN = os.getenv("BOT_TOKEN", "7345643006:AAGM2y4lpGwQeAJ12cyrMjRSXROMRV9Dl1o") 
OWNER_ID = int(os.getenv("OWNER_ID", "7383553662")) 
LOGS_MAKER_UBOT = int(os.getenv("LOGS_MAKER_UBOT", "-1002171403395"))
BLACKLIST_CHAT = list(map(int, os.getenv("BLACKLIST_CHAT", "-1002315219516 -1002239800569").split()))
MAX_BOT = int(os.getenv("MAX_BOT", "65"))
DEVS = list(map(int, os.getenv("DEVS", "7383553662 6568046805").split()))
RMBG_API = os.getenv("RMBG_API", "b5ZnjZ2nUUpbdEHfcrWdjWbC")
AI_GOOGLE_API = os.getenv("AI_GOOGLE_API", "AIzaSyAM4A7L0Qj3loDZDupt0X74PDne6Tx2YLA")
MONGO_URL = os.getenv(
    "MONGO_URL",
    "mongodb+srv://titar:fadhil123@cluster0.ppxf2yw.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0",
)
